...
function UserStatsScreen({ navigation }) {
  ...
  const fetchAllContent = async (forceRefresh = false) => {
    ...
    try {
      ...
      do {
        ...
        allUserPosts = [...allUserPosts, ...response.posts];
        cursor = response.cursor;
        pageCount++;
        
        setProgress(Math.min(0.4, pageCount * 0.02)); // Adjust progress based on realistic page counts
        setStatus(`Fetched ${allUserPosts.length} posts (page ${pageCount})`);
      ...
      do {
        ...
        allUserComments = [...allUserComments, ...response.posts.filter(p => p.authored_by_user)];
        cursor = response.cursor;
        pageCount++;
        
        setProgress(Math.min(0.8, 0.4 + (pageCount * 0.02))); // Second half for comments
        setStatus(`Fetched ${allUserComments.length} comments (page ${pageCount})`);
  ...
  const processAnalytics = (posts, comments) => {
    ...
    const oldestContent = allContent.length > 0 ? allContent.reduce((oldest, item) => {
      const itemDate = new Date(item.created_at);
    ...
    <View style={{ backgroundColor: colors.background, flex: 1 }}>
      ...
      <ScrollView style={{ padding: 16 }} showsVerticalScrollIndicator={false}>
        ...
        {loading && (
          <Card style={{ marginBottom: 16 }}>
            <Card.Content>
              <Text variant="titleSmall" style={{ marginBottom: 8 }}>Collecting Data...</Text>
